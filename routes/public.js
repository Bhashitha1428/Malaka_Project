...
router.post('/getReports', function (req, res) {
    ...
    //res.redirect(env.frontendPath() + '/profile');
    console.log(`Getting Projects `);
    console.log(req.body.developerId);
   User.findById(req.body.developerId)
       .then(user=>{
           console.log(user);
           user.reportUsers.push(req.body.userId)
           user.save()
           User
           .findOneAndUpdate({_id:req.body.developerId},{
             $inc:{
               
               count:1  //to increment report count value by 1
                },
...
//Geting deatails reported user
router.get('/reportedUserDetails',(req,res)=>{

  
  User.findById(req.body.developerId)
      .populate("reportUsers","name")
      .select('name')
      .then(result=>{
          res.status(200).json({
              ReportedUser:result,
      ...
      .catch(err=>{
          res.status(500).json({err:err,state:false})
      })
})
